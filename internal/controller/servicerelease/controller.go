...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.applyResources(ctx, dpClient, serviceRelease); err != nil {
...
// applyResources applies all resources from ServiceRelease to the dataplane
func (r *Reconciler) applyResources(ctx context.Context, dpClient client.Client, serviceRelease *choreov1.ServiceRelease) error {
	logger := log.FromContext(ctx)
	...
	for _, resource := range serviceRelease.Spec.Resources {
		...
		r.addOwnershipLabels(obj, serviceRelease)
...
func (r *Reconciler) addOwnershipLabels(obj *unstructured.Unstructured, serviceRelease *choreov1.ServiceRelease) {
	...
	resourceLabels[labels.LabelKeyProjectName] = serviceRelease.Spec.Owner.ProjectName
	resourceLabels[labels.LabelKeyComponentName] = serviceRelease.Spec.Owner.ComponentName
	resourceLabels[labels.LabelKeyEnvironmentName] = serviceRelease.Spec.EnvironmentName
...
func (r *Reconciler) applyResource(ctx context.Context, dpClient client.Client, obj *unstructured.Unstructured) error {
	...
	return dpClient.Patch(ctx, obj, client.Apply, client.ForceOwnership, client.FieldOwner("servicerelease-controller"))
}
