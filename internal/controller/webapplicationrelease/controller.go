...
// Reconciler reconciles a WebApplicationRelease object
...
// +kubebuilder:rbac:groups=core.choreo.dev,resources=webapplicationreleases/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=core.choreo.dev,resources=webapplicationreleases/finalizers,verbs=update
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Fetch the WebApplicationRelease instance
	webApplicationRelease := &choreov1.WebApplicationRelease{}
	if err := r.Get(ctx, req.NamespacedName, webApplicationRelease); err != nil {
		if apierrors.IsNotFound(err) {
			logger.Info("WebApplicationRelease resource not found. Ignoring since it must be deleted.")
			return ctrl.Result{}, nil
		}
		logger.Error(err, "Failed to get WebApplicationRelease")
		return ctrl.Result{}, err
	}

	// Get dataplane client for the environment
	dpClient, err := r.getDPClient(ctx, webApplicationRelease.Spec.EnvironmentName)
	if err != nil {
	...
	if err := r.applyResources(ctx, dpClient, webApplicationRelease); err != nil {
		logger.Error(err, "Failed to apply resources to dataplane")
		return ctrl.Result{}, err
	}

	logger.Info("Successfully applied WebApplicationRelease resources to dataplane")
	return ctrl.Result{}, nil
}
...
// applyResources applies all resources from WebApplicationRelease to the dataplane
func (r *Reconciler) applyResources(ctx context.Context, dpClient client.Client, webApplicationRelease *choreov1.WebApplicationRelease) error {
	logger := log.FromContext(ctx)

	for _, resource := range webApplicationRelease.Spec.Resources {
		...
		r.addOwnershipLabels(obj, webApplicationRelease)
...
func (r *Reconciler) addOwnershipLabels(obj *unstructured.Unstructured, webApplicationRelease *choreov1.WebApplicationRelease) {
	...
	resourceLabels[labels.LabelKeyProjectName] = webApplicationRelease.Spec.Owner.ProjectName
	resourceLabels[labels.LabelKeyComponentName] = webApplicationRelease.Spec.Owner.ComponentName
	resourceLabels[labels.LabelKeyEnvironmentName] = webApplicationRelease.Spec.EnvironmentName
...
func (r *Reconciler) applyResource(ctx context.Context, dpClient client.Client, obj *unstructured.Unstructured) error {
	...
	return dpClient.Patch(ctx, obj, client.Apply, client.ForceOwnership, client.FieldOwner("webapplicationrelease-controller"))
}

// SetupWithManager sets up the controller with the Manager.
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&choreov1.WebApplicationRelease{}).
		Named("webapplicationrelease").
		Complete(r)
}
