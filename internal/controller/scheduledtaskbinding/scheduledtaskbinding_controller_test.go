...
var _ = Describe("ScheduledTaskBinding Controller", func() {
	Context("When reconciling a resource", func() {
		...
		scheduledtaskbinding := &corev1.ScheduledTaskBinding{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind ScheduledTaskBinding")
			err := controller.k8sClient.Get(ctx, typeNamespacedName, scheduledtaskbinding)
			if err != nil && errors.IsNotFound(err) {
				resource := &corev1.ScheduledTaskBinding{
