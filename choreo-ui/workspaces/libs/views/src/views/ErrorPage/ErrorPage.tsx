...
export function ErrorPage(props: ErrorPageProps) {
  ...
  return (
    <Box padding={theme.spacing(8)} display="flex" flexDirection="column" alignItems="center" justifyContent="center" gap={theme.spacing(4)}>
      {image}
      <Typography variant="h2">{title}</Typography>
      <Typography variant="body1" color={theme.pallet.text.secondary}>{description}</Typography>
    </Box>
  );
}
...
export function PresetErrorPage(props: PresetErrorPageProps) {
  const { preset } = props;
  return (
    <ErrorPage description={getDescription(preset)} title={getTitle(preset)} image={getImage(preset)} />
  );
}
